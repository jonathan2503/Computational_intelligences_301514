...
FRIC  = pygame.font.Font('freesansbold.ttf', 52)
H =Font.render("H", False, BLACK,None)
L =Font.render("L", False, BLACK ,None )
R =Font.render("RST", False, BLACK,None )
AI =FRIC.render("AI WIN", False, GOLD,None )
PL =FRIC.render("YOU WIN", False, GOLD,None )
pawn = {
...
class Board:
    ...
    def get_position (pos):
        ...
        if pos == 25:
            return 800, central_dot + 2*radius +100
    ...
    def draw_pawn (self,color,type,pos): #,type,cio ,x,y,color
        ...
        if type == 5: #'C_H'
            pygame.draw.circle(self, BLACK, list(Board.get_position(pos)), radius+1,0)
            pygame.draw.circle(self, color, list(Board.get_position(pos)), radius+1,0)
            self.blit(H,(x-10,y-10))
        if type == 6: #'C_L'
            pygame.draw.circle(self, BLACK, list(Board.get_position(pos)), radius +1,0)
            pygame.draw.circle(self, color, list(Board.get_position(pos)), radius+1,0)
        ...
        if type == 8: #'CH_L'
            ...
            self.blit(L_W,(x-10,y-10))
        if type == 9: #'R'
            pygame.draw.circle(self, GOLD, list(Board.get_position(25)), radius,0)
            self.blit(R,(770, central_dot + 2*radius +89))
               
    def reset (self):
        self.fill(BLACK)
        Board.board_set_up (self); 

    def AI_WIN (self):
        self.blit(AI,(690, central_dot + 2*radius +160)) 

    def PL_WIN (self):
        self.blit(PL,(690, central_dot + 2*radius +160)) 
...
class Button():
    ...
    def delete(self,win):
        pygame.draw.circle(win, BLACK, list(Board.get_position(self.Pos)), radius+2,0)
...
class Table_button():
    ...
    def set_pos (self, pos,type, color ):
         
        Board.draw_pawn(self.win,color, type, pos )
        self.active =  False
